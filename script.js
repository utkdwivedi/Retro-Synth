/* -------------------------------------------------------
   RETRO SYNTH � Single bundled script
   Contains: riffwave.js + sfxr.js + app logic
   ------------------------------------------------------- */

// ---------------- RIFFWAVE (WAV encoder) ----------------
/* RIFFWAVE.js — PCM WAV encoder for browser/node
   Original by Pedro Ladaria · MIT License
   Minimal self-contained version for jsfxr compatibility */
...
var RIFFWAVE = function(data) {
  ...
  this.header = {
    ...
    numChannels   : 1,
    sampleRate    : 44100,
    byteRate      : 0,                      // filled in
    blockAlign    : 0,                      // filled in
    bitsPerSample : 8,
    subChunk2Id   : [0x64,0x61,0x74,0x61], // "data"
    subChunk2Size : 0                       // filled in
  };
...
RIFFWAVE.prototype.Make = function(data) {
  if (!(data instanceof Array)) data = Array.prototype.slice.call(data);
  this.data = data;
  var h = this.header;

  h.sampleRate    = this.sampleRate    || h.sampleRate;
  h.bitsPerSample = this.bitsPerSample || h.bitsPerSample;
  h.numChannels   = this.numChannels   || h.numChannels;

  h.blockAlign    = (h.numChannels * h.bitsPerSample) >> 3;
  h.byteRate      = h.sampleRate * h.blockAlign;
  h.subChunk2Size = data.length * (h.bitsPerSample >> 3);
  h.chunkSize     = 36 + h.subChunk2Size;

  this.wav = h.chunkId.concat(
    this.u32ToArray(h.chunkSize),
    h.format,
    h.subChunk1Id,
    this.u32ToArray(h.subChunk1Size),
    this.u16ToArray(h.audioFormat),
    this.u16ToArray(h.numChannels),
    this.u32ToArray(h.sampleRate),
    this.u32ToArray(h.byteRate),
    this.u16ToArray(h.blockAlign),
    this.u16ToArray(h.bitsPerSample),
    h.subChunk2Id,
    this.u32ToArray(h.subChunk2Size),
    data
  );
  ...
  while (i < bytes.length) {
    ...
    base64 += chars[((b1 & 0xf) << 2) | (b2 >> 6)];
    base64 += chars[b2 & 0x3f];
  }
  // Pad
  ...
  this.dataURI = 'data:audio/wav;base64,' + base64;
};


// ---------------- SFXR (sound synthesis) ----------------
// Wave shapes
var SQUARE = 0;
var SAWTOOTH = 1;
var SINE = 2;
...
// ---------------- APP LOGIC -----------------------------
// RETRO SYNTH — JSFXR Synthesizer (full-screen)

const SLIDER_CONFIG = [
  // ── Column 1: Envelope + Frequency ──────────────────
  { id: 'p_env_attack',    name: 'Attack Time',    col: 1 },
  { id: 'p_env_sustain',   name: 'Sustain Time',   col: 1 },
  { id: 'p_env_punch',     name: 'Sustain Punch',  col: 1 },
  { id: 'p_env_decay',     name: 'Decay Time',     col: 1 },
  { id: 'p_base_freq',     name: 'Start Freq',     col: 1 },
  { id: 'p_freq_limit',    name: 'Min Freq Cut',   col: 1 },
  { id: 'p_freq_ramp',     name: 'Slide',          col: 1 },
  { id: 'p_freq_dramp',    name: 'Delta Slide',    col: 1 },
  { id: 'p_vib_strength',  name: 'Vibrato Depth',  col: 1 },
  { id: 'p_vib_speed',     name: 'Vibrato Speed',  col: 1 },
  { id: 'p_duty',          name: 'Duty Cycle',     col: 1 },

  // ── Column 2: Modulation + Filter ───────────────────
  { id: 'p_duty_ramp',     name: 'Duty Sweep',     col: 2 },
  { id: 'p_arp_mod',       name: 'Arp Freq Mult',  col: 2 },
  { id: 'p_arp_speed',     name: 'Arp Speed',      col: 2 },
  { id: 'p_repeat_speed',  name: 'Repeat Rate',    col: 2 },
  { id: 'p_pha_offset',    name: 'Flanger Offset', col: 2 },
  { id: 'p_pha_ramp',      name: 'Flanger Sweep',  col: 2 },
  { id: 'p_lpf_freq',      name: 'LPF Cutoff',     col: 2 },
  { id: 'p_lpf_ramp',      name: 'LPF Sweep',      col: 2 },
  { id: 'p_lpf_resonance', name: 'LPF Resonance',  col: 2 },
  { id: 'p_hpf_freq',      name: 'HPF Cutoff',     col: 2 },
  { id: 'p_hpf_ramp',      name: 'HPF Sweep',      col: 2 },
];

let synthParams, currentSound, canvas, ctx;

document.addEventListener('DOMContentLoaded', () => {
  synthParams = new Params();
  ...
  synthParams.pickupCoin();
  updateUIFromParams();
  generateSound(false);
});

/* ── Build sliders into two columns ─────────────── */
function buildSliders() {
  const col1 = document.getElementById('sliders-col-1');
  const col2 = document.getElementById('sliders-col-2');
  if (!col1 || !col2) return;

  SLIDER_CONFIG.forEach(cfg => {
    const target = cfg.col === 1 ? col1 : col2;

    const row = document.createElement('div');
    row.className = 'slider-row';
    row.innerHTML = `
      <label>${cfg.name}</label>
      <input type="range" id="synth_${cfg.id}" min="-1000" max="1000" value="0">
    ...
    input.addEventListener('input', e => {
      ...
      synthParams[cfg.id] = val;
      row.querySelector('.slider-val').textContent = val.toFixed(2);
      generateSound(false);
    });
    input.addEventListener('change', () => generateSound(true));
  });
}
...
function bindEvents() {
  // Wave type
  document.querySelectorAll('input[name="wave_type"]').forEach(r => {
    r.addEventListener('change', e => {
      if (e.target.checked) {
        synthParams.wave_type = parseInt(e.target.value, 10);
        generateSound(true);
      }
    });
  });
  ...
  document.querySelectorAll('.gen-btn').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      if (gen === 'random') {
        synthParams = new Params();
        synthParams.mutate(); synthParams.mutate(); synthParams.mutate();
      } else if (typeof synthParams[gen] === 'function') {
        synthParams[gen]();
      }
      updateUIFromParams();
      generateSound(true);
      setStatus(gen.toUpperCase());
    });
  });

  // Play button
  document.getElementById('play-btn')
    ?.addEventListener('click', () => generateSound(true));

  // Close box — hide/show window (cosmetic)
  document.querySelector('.mac-close-box')
    ?.addEventListener('click', () => {
      const w = document.getElementById('synth-window');
      if (w) w.style.opacity = w.style.opacity === '0' ? '1' : '0';
    });
}
...
function updateUIFromParams() {
  document.querySelectorAll('input[name="wave_type"]').forEach(r => {
    r.checked = (parseInt(r.value, 10) === synthParams.wave_type);
  });
  SLIDER_CONFIG.forEach(cfg => {
    const input = document.getElementById('synth_' + cfg.id);
    if (input) {
      const val = synthParams[cfg.id] || 0;
...
/* ── Generate + play sound ──────────────────────── */
function generateSound(play = true) {
  if (!window.sfxr) return;
  synthParams.sound_vol  = 0.5;
  synthParams.sample_rate = 44100;
  synthParams.sample_size = 8;

  window.currentSynthParams = synthParams;   // expose for external use
  currentSound = sfxr.toWave(synthParams);
  ...
  drawWaveform(currentSound.buffer);
}
...
function drawDataLine(data, w, h) {
  ctx.beginPath();
  const step = Math.max(1, Math.floor(data.length / w));
  for (let i = 0; i < w; i++) {
    const di = i * step;
    if (di >= data.length) break;
    const y = (h / 2) + (data[di] * (h / 2.5));
